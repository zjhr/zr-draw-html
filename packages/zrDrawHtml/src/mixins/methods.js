...
export default {
	...
	methods: {
		...
		getTableGatherRelationFun(cell) {
			// 获取TableGather缓存的信息
			const { source, target, value } = cell
			const sTableRowIndex = ~~value.getAttribute('sourceRow') - 1
			const tTableRowIndex = ~~value.getAttribute('targetRow') - 1
			...
			const sTableRowId =
				sTableRowIndex !== -1
					? source.value.data.children[sTableRowIndex].fieldId
					: undefined
			const tTableRowId =
				tTableRowIndex !== -1
					? target.value.data.children[tTableRowIndex].fieldId
					: undefined
			const sTableRow =
				sTableRowIndex !== -1 ? sTable.children[sTableRowId] : undefined
			const tTableRow =
				sTableRowIndex !== -1 ? tTable.children[tTableRowId] : undefined
			return {
				sTableRowIndex,
				tTableRowIndex,
				sTable,
		...
		// 处理连线/删除连线的方法
		lineCallBackFun(
			{ sTable, tTable, sTableRowIndex, tTableRowIndex, sTableRow, tTableRow },
			name,
			callback
		) {
			const handleFun = (obj, type) => {
				...
				const toIndex = obj.findIndex(
					(v) => v.tableId === tTable.tableId && v.fieldId === fieldId
				)
				const newObj = {
					type,
					sTableRowIndex,
		...
		doubleClickFun(cell) {
			...
			if (cell) {
				if (cell.edge) {
					...
					// 非线
					const tableIndex = this.tableArr.findIndex(
						(v) => v.tableId === cell.id
					)
		...
		cellsResizedFun(cell) {
			// 调整大小回调
			// debugger;
			const { id, geometry, value } = cell
		...
		cellsAddedFun(lineObj) {
			// 添加连线回调
			if (!~lineObj.id.indexOf('~')) {
				// 判断是否触发式的连线事件
				...
				const verifyFun = (obj, callback) => {
					if (
						obj.to.some(
							(v) => v.tableId === to.tableId && v.fieldId === to.fieldId
						)
					) {
		...
		autoWidthFun(tableName, children) {
			// 适应宽度方法
			let width = 160
			const byteFun = (str) => {
				let bytesCount = 0
		...
		// 添加表方法
		addTable(data) {
			const { tableId, tableName, children: child, state, width } = data
